...
class QuotationAttribution:
    ...
    def get_representation(self, quotes, entities, tokens, doLowerCase=True):
        ...
        def convert_word(word):
            if word in ["[ALTQUOTE]", "[PAR]", "[QUOTE]"]:
                word = word

            elif doLowerCase:
                word = f"[CAP] {word.lower()}" if word[0].lower() != word[0] else word.lower()
            return word
        ...
        for q_id, (start_tok, end_tok) in enumerate(quotes):
            ...
            if tot_toks > 500:
                raise ValueError(f"Quotation window is unexpectedly long: {tot_toks}")
